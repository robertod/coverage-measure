...
if ~isfield(options, 'edge_alpha')
    options.edge_alpha = 0;
end
...
if isempty(face_vertex_color)
    h = patch('vertices',vertex,'faces',face,'facecolor',[face_color face_color face_color],'edgecolor',[edge_color edge_color edge_color],'edgealpha',edge_alpha);
else
    ...
    h = patch('vertices',vertex,'faces',face,'FaceVertexCData',face_vertex_color, 'FaceColor',shading_type,'edgealpha',edge_alpha);
end
colormap gray(256);
lighting phong;
camlight infinite; 
